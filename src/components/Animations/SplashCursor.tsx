...
function compileShader(gl: WebGLRenderingContext | WebGL2RenderingContext, type: number, source: string, keywords: string[] | null = null): WebGLShader | null {
...
function createProgram(gl: WebGLRenderingContext | WebGL2RenderingContext, vertexShader: WebGLShader | null, fragmentShader: WebGLShader | null): WebGLProgram | null {
...
export default function SplashCursor({
  ...
  SPLAT_RADIUS = 0.2,
  SPLAT_FORCE = 6000,
  SHADING = false, // Disabled by default for ☝️🚀 performance
  ...
  useEffect(() => {
    ...
    const copyProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
    ...
    const clearProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
    ...
    const splatProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision highp float;
    ...
    const advectionProgram = new Material(gl, baseVertexShader, `
      precision highp float;
      precision highp sampler2D;
      varying vec2 vUv;
    ...
    const divergenceProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
    ...
    const curlProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
    ...
    const vorticityProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision highp float;
    ...
    const pressureProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
    ...
    const gradienSubtractProgram = new Program(gl, baseVertexShader, compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      `
      precision mediump float;
    ...
    function updateKeywords() {
      ...
      const advectionKeywords: string[] = [];
      if (!ext.supportLinearFiltering) advectionKeywords.push('MANUAL_FILTERING');
      advectionProgram.setKeywords(advectionKeywords);
    }
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      if (splatProgram.uniforms.radius) {
        const rad = correctRadius(config.SPLAT_RADIUS / 100);
        if (rad !== null) gl.uniform1f(splatProgram.uniforms.radius, rad);
      }
      blit(velocity.write);
      velocity.swap();
    ...
    function generateColor(): ColorRGB {
      // THE BLOODLINE THEME: Only Reds and Dark Reds
      const isBlack = Math.random() < 0.2;
      if (isBlack) return { r: 0.05, g: 0, b: 0 };
      
      const r = 0.4 + Math.random() * 0.6; // Strong red base
      return { r, g: 0, b: 0 };
    }
